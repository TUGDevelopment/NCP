...
public partial class _Default2 : System.Web.UI.Page {
    ...
    protected void lblUserName_OnInit(object sender, EventArgs e)
    {
        if (ASPxGridView1.IsNewRowEditing)
        {
            ((ASPxLabel)sender).Text = Session["UserName"].ToString();
        }
        else
        {
            GridViewEditItemTemplateContainer templateContainer = ((ASPxLabel)sender).NamingContainer as GridViewEditItemTemplateContainer;
            ((ASPxLabel)sender).Text = templateContainer.Text;
        }
    }
